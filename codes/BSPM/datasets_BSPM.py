...
class Tokyo(object):
    ...
    def split_data(self, raw_matrix, time_matrix, random_seed=0):
        ...
        for user_id in range(self.user_num):
            ...
            for i in range(len(train_place)):
                ...
                train_matrix[user_id, train_place[i]] = 1
                self.trainUser.append(user_id)
                self.trainItem.append(train_place[i])
            ...
            train_positive.append(train_place)
        self.user_POI_Graph = sparse.csr_matrix((np.ones(len(self.trainUser)), (self.trainUser, self.trainItem)), shape=(self.user_num, self.poi_num))
        adj_mat = self.user_POI_Graph
        lm = BSPM(adj_mat)
        lm.train()

        batch_ratings = convert_sp_mat_to_sp_tensor(adj_mat)
        rating = lm.getUsersRating(batch_ratings)
        rating[rating >= 0.05] = 1.0
        rating[rating < 0.05] = 0.0
        exclude_index = []
        exclude_items = []
        include_index = []
        include_items = []
        for i in range(self.user_num):
            exclude_index.extend([i] * len(test_positive[i]))
            exclude_items.extend(test_positive[i])

            exclude_index.extend([i] * len(val_positive[i]))
            exclude_items.extend(val_positive[i])

            include_index.extend([i] * len(train_positive[i]))
            include_items.extend(train_positive[i])
        rating[exclude_index, exclude_items] = 0.0
        rating[include_index, include_items] = 1.0
        ui, ii = torch.nonzero(rating.to("cpu"),as_tuple=True)
        aug_train_mat = sparse.csr_matrix((np.ones(len(ui)), (ui, ii)), shape=(self.user_num, self.poi_num))
        return aug_train_mat, test_positive, val_positive
    ...
    def read_poi_coos(self,near_POI_num):
        ...
        self.nearPOI = np.argpartition(self.dist_matrix,near_POI_num)
        self.nearPOI = self.nearPOI[:,:near_POI_num]
        return place_coords
...
class Philadelphia(object):
    ...
    def split_data(self, raw_matrix, time_matrix, random_seed=0):
        ...
        batch_ratings = convert_sp_mat_to_sp_tensor(adj_mat)
        rating = lm.getUsersRating(batch_ratings)
        rating[rating > 0.08] = 1.0
        rating[rating < 0.08] = 0.0
        exclude_index = []
        exclude_items = []
        include_index = []
        include_items = []
...
class NewYork(object):
    ...
    def split_data(self, raw_matrix, time_matrix, random_seed=0):
        ...
        batch_ratings = convert_sp_mat_to_sp_tensor(adj_mat)
        rating = lm.getUsersRating(batch_ratings)
        rating[rating >= 0.06] = 1.0
        rating[rating < 0.06] = 0.0
        exclude_index = []
        exclude_items = []
        include_index = []
        include_items = []
