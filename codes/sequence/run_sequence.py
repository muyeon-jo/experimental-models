...
import os
import datasets_sequence as datasets
from batches_sequence import *
from model_sequence import *
import time
import random
import validation_sequence as val
import torch.cuda
import torch
from save_sequence import save_experiment_result

def train(train_positive, test_positive, val_positive, dataset,args):
    ...
    for e in range(args.epochs):
        ...
        for buid in idx:
            optimizer.zero_grad()
            user_history , train_data, train_label, ptoc, ctof = train_batch(train_positive, num_items, buid, args.num_ng)
            temp = user_history.expand([len(train_label),len(user_history[0])])
            
            prediction = model(temp, train_data, dataset.nearPOI,ptoc, ctof)
        ...
        if (e+1)%5 == 0:
            ...
            with torch.no_grad():
                # model.BSPM()
                start_time = int(time.time())
                precision_v, recall_v, hit_v, precision_t, recall_t, hit_t = val.validation(model,num_users,test_positive,val_positive,train_positive,k_list,dataset.nearPOI)
...
def run(dataset,arg):
    train_positive, test_positive, val_positive, place_coords = dataset.generate_data(0,args.near_num)
    print("train data generated")
    
    print("train start")
    train(train_positive, test_positive, val_positive, dataset,arg)
if __name__ == '__main__':
    ...
    parser.add_argument('-l','--lamda', type=float, default=0.0, help='regularization weight')
    parser.add_argument('-ng', '--num_ng', type=int, default=4, help='number of negative sample')
    parser.add_argument('-hi','--hidden_dim', type=int, default=64, help='hidden_dimmension')
